...
$formAction = $isEdit ? BASE_URL.'kendaraan/update/'.$kendaraan['kodekendaraan'] : BASE_URL.'kendaraan/store';
$val = fn($key,$default='') => $old[$key]??($isEdit?$kendaraan[$key]??$default:$default);
?>
<style>
  ...
  <div class="card-form">
    ...
    <form action="<?= $formAction ?>" method="post" class="row">
      ...
      <div class="col-lg-6">
        ...
        <input type="text" name="kodekendaraan" class="form-control" value="<?= htmlspecialchars($val('kodekendaraan')) ?>" required>
      </div>

      <div class="col-lg-6">
        <label class="form-label">Nama Kendaraan</label>
        <input type="text" name="namakendaraan" class="form-control" value="<?= htmlspecialchars($val('namakendaraan')) ?>" required>
      </div>

      <div class="col-lg-6">
        <label class="form-label">Plat Nomor</label>
        <input type="text" name="platnomor" class="form-control" value="<?= htmlspecialchars($val('platnomor')) ?>" required>
      </div>
      ...
      <div class="col-lg-6">
        ...
        <select name="tipe" class="form-control">
          ...
          <option value="Mobil" <?= $val('tipe')=='Mobil'?'selected':'' ?>>Mobil</option>
          <option value="Motor" <?= $val('tipe')=='Motor'?'selected':'' ?>>Motor</option>
          <option value="Truk" <?= $val('tipe')=='Truk'?'selected':'' ?>>Truk</option>
          <option value="Lainnya" <?= $val('tipe')=='Lainnya'?'selected':'' ?>>Lainnya</option>
        </select>
      </div>

      <div class="col-12 mt-4 d-flex justify-content-end">
        <a href="<?= BASE_URL ?>kendaraan" class="btn-cancel">Kembali</a>
