...
$formAction = $isEdit
  ? BASE_URL . 'gudang/update/' . ($gudang['id'] ?? '')
  : BASE_URL . 'gudang/store';

$val = function ($key, $default = '') use ($gudang, $old, $isEdit) {
  if (!empty($old) && isset($old[$key]))
    return $old[$key];
  if ($isEdit && !empty($gudang) && isset($gudang[$key]))
    return $gudang[$key];
  return $default;
};
?>
  ...
  <div class="card-farm w-100">
    ...
    <form action="<?= $formAction ?>" method="post" enctype="multipart/form-data" class="row g-4 mt-2" novalidate>
      ...
      <div class="col-lg-6 col-md-6">
        ...
        <input type="text" name="kodegudang" class="form-control" placeholder="Contoh: GD-001"
          value="<?= $this->e($val('kodegudang')) ?>" required>
      </div>

      <div class="col-lg-6 col-md-6">
        <label class="form-label">Nama Gudang <span class="text-danger">*</span></label>
        <input type="text" name="namagudang" class="form-control" placeholder="Masukkan nama gudang "
          value="<?= $this->e($val('namagudang')) ?>" required>
      </div>

      <div class="col-lg-6 col-md-6">
        <label class="form-label">Golongan</label>
        <input type="text" name="golongan" class="form-control"
          value="<?= $this->e($val('golongan')) ?>">
      </div>
      <div class="col-lg-6 col-md-6">
        <label class="form-label">keterangan</label>
        <input type="text" name="keterangan" class="form-control"
          value="<?= $this->e($val('keterangan')) ?>">
      </div>

      <div class="col-12 d-flex flex-wrap justify-content-end gap-3 mt-5">
        <a href="<?= BASE_URL ?>gudang" class="btn btn-outline-secondary px-5 py-2 rounded-pill fw-semibold">
