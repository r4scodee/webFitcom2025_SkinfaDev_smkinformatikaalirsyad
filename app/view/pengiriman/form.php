...
$formAction = $isEdit
  ? BASE_URL . 'pengiriman/update/' . ($pengiriman['id'] ?? '')
  : BASE_URL . 'pengiriman/store';

$val = function ($key, $default = '') use ($pengiriman, $old, $isEdit) {
  if (!empty($old) && isset($old[$key]))
    return $old[$key];
  if ($isEdit && !empty($pengiriman) && isset($pengiriman[$key]))
    return $pengiriman[$key];
  return $default;
};
?>
  ...
  <div class="card-farm w-100">
    ...
    <form action="<?= $formAction ?>" method="post" enctype="multipart/form-data" class="row g-4 mt-2" novalidate>
      ...
      <div class="col-lg-6 col-md-6">
        <label class="form-label">Kode Pengiriman <span class="text-danger">*</span></label>
        <input type="text" name="kodekirim" class="form-control" placeholder="Contoh: KRM-001"
          value="<?= $this->e($val('kodekirim')) ?>" required>
      </div>

      <div class="col-lg-6 col-md-6">
        <label class="form-label">Tanggal Pengiriman <span class="text-danger">*</span></label>
        <input type="text" name="tglkirim" class="form-control" placeholder="Masukkan tanggal pengiriman"
          value="<?= $this->e($val('tglkirim')) ?>" required>
      </div>

      <div class="col-lg-6 col-md-6">
        <label class="form-label">nopol</label>
        <input type="text" name="nopol" class="form-control"
          value="<?= $this->e($val('nopol')) ?>" placeholder="Masukkan nopol">
      </div>
      <div class="col-lg-6 col-md-6">
        <label class="form-label">totalqty</label>
        <input type="text" name="totalqty" class="form-control"
          value="<?= $this->e($val('totalqty')) ?>" placeholder="Masukkan total qty">
      </div>

      <div class="col-12 d-flex flex-wrap justify-content-end gap-3 mt-5">
        <a href="<?= BASE_URL ?>pengiriman" class="btn btn-outline-secondary px-5 py-2 rounded-pill fw-semibold">
